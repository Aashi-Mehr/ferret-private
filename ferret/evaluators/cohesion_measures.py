...
from .utils_from_soft_to_discrete import (
    ...
    get_discrete_explanation,
)


class NSegments_CohesionEvaluation(BaseEvaluator):
    NAME = "n_segments_cohesion"
    SHORT_NAME = "nseg_cohes"
    ...
    def _compute_n_segments(self, arr: np.array) -> int:
        ...
        # Compute differences; a rising edge (False->True) will result in 1.
        transitions = np.diff(padded.astype(int)) == 1

        return int(np.sum(transitions))


    def _compute_n_segments_score(self, g, r):
        # Option 1: Reciprocal Form (Currently used)
        # Alternative: Exponential Decay Form (Not implemented)
        return 1 / (1 + np.abs(g - r))


    def _compute_average_score(self, scores):
    ...
    def compute_evaluation(
        ...
        **evaluation_args
        ...
        human_rationale = explanation.rationale
        ...
        # Gets the function which interprets thresholds
        ...
        for v in thresholds:
            ...
            if top_tokens_g:
                ...
                rationale_n_segments = self._compute_n_segments(np.array(top_tokens_r))
        ...
        return EvaluationMetricOutput(
            self,
            self._compute_average_score(n_segment_scores)
        )
